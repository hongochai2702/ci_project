          ...
          <div class="tab-content">
            <div class="tab-pane active" id="tab-general">
              <ul class="nav nav-tabs" id="language">
                ...
                <li><a href="#language<?php echo $language['language_id']; ?>" data-toggle="tab"><img src="public/image/flags/<?php echo $language['code']; ?>.png" title="<?php echo $language['name']; ?>" /> <?php echo $language['name']; ?></a></li>
                <?php } ?>
              </ul>
              <div class="tab-content"><?php foreach ($languages as $language) { ?>
                <div class="tab-pane" id="language<?php echo $language['language_id'] ?>">
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-10">
                      <textarea name="product_description[<?php echo $language['language_id']; ?>][description]" placeholder="<?php echo $entry_description; ?>" id="input-description<?php echo $language['language_id']; ?>" data-toggle="summernote" data-lang="<?php echo $language['code']; ?>" class="form-control"><?php echo isset($product_description[$language['language_id']]) ? $product_description[$language['language_id']]['description'] : ''; ?></textarea>
                    </div>
                  </div>
                  <div class="form-group required">
            ...
            <div class="tab-pane" id="tab-links">
              ...
              <div class="form-group">
                ...
                <div class="col-sm-10">
                  <div class="well well-sm" style="height: 150px; overflow: auto;"> <?php foreach ($stores as $store) { ?>
                    <div class="checkbox">
                  ...
                  <tbody>
                    ...
                    <tr id="attribute-row<?php echo $attribute_row; ?>">
                      ...
                      <td class="text-left"><?php foreach ($languages as $language) { ?>
                        <div class="input-group"><span class="input-group-addon"><img src="public/image/flags/<?php echo $language['code']; ?>.png" title="<?php echo $language['name']; ?>" /></span>
                ...
                <table class="table table-bordered table-hover">
                 ...
                 <?php foreach ($stores as $store) { ?>
                  <tr>
                    ...
                    <td class="text-left"><?php foreach ($languages as $language) { ?>
                      <div class="input-group"><span class="input-group-addon"><img src="public/image/flags/<?php echo $language['code']; ?>.png" title="<?php echo $language['name']; ?>" /></span>
                        <input type="text" name="product_seo_url[<?php echo $store['store_id'];?>][<?php echo $language['language_id']; ?>]" value="<?php if(isset($product_seo_url[$store['store_id']][$language['language_id']])) { ?><?php echo $product_seo_url[$store['store_id']][$language['language_id']]; ?><?php } ?>" placeholder="<?php echo $entry_keyword; ?>" class="form-control" />
                      </div>
                  ...
                  <tr>
                    ...
                    <td class="text-left"><select name="product_layout[<?php echo $store['store_id'] ?>]" class="form-control">
                        <option value=""></option>
                          ...
                          <?php if($product_layout[$store['store_id']] & $product_layout[$store['store_id']] == $layout['layout_id']) { ?>
...
$('input[name=\'manufacturer\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'catalog/manufacturer/autocomplete?user_token=<?php echo $user_token; ?>&filter_name=' +  encodeURIComponent(request),
			dataType: 'json',
...
$('input[name=\'category\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'catalog/category/autocomplete?user_token=<?php echo $user_token; ?>&filter_name=' +  encodeURIComponent(request),
			dataType: 'json',
			success: function(json) {
				response($.map(json, function(item) {
					return {
...
$('input[name=\'download\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'catalog/download/autocomplete?user_token=<?php echo $user_token; ?>&filter_name=' +  encodeURIComponent(request),
			dataType: 'json',
			success: function(json) {
				response($.map(json, function(item) {
					return {
...
$('input[name=\'related\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'catalog/product/autocomplete?user_token=<?php echo $user_token; ?>&filter_name=' +  encodeURIComponent(request),
			dataType: 'json',
			success: function(json) {
				response($.map(json, function(item) {
					return {
...
function addAttribute() {
	...
	<?php foreach ($languages as $language) { ?>
	html += '<div class="input-group"><span class="input-group-addon"><img src="public/image/flags/<?php echo $language['code'] ?>}/<?php echo $language['code'] ?>}.png" title="<?php echo $language['name'] ?>" /></span><textarea name="product_attribute[' + attribute_row + '][product_attribute_description][<?php echo $language['language_id'] ?>}][text]" rows="5" placeholder="<?php echo $entry_text; ?>" class="form-control"></textarea></div>';
    <?php } ?>
	html += '  </td>';
...
function attributeautocomplete(attribute_row) {
	$('input[name=\'product_attribute[' + attribute_row + '][name]\']').autocomplete({
		'source': function(request, response) {
			$.ajax({
				url: 'catalog/attribute/autocomplete?user_token=<?php echo $user_token; ?>&filter_name=' +  encodeURIComponent(request),
				dataType: 'json',
				success: function(json) {
					response($.map(json, function(item) {
						return {
...
$('input[name=\'option\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'catalog/option/autocomplete?user_token=<?php echo $user_token; ?>&filter_name=' +  encodeURIComponent(request),
			dataType: 'json',
			success: function(json) {
				response($.map(json, function(item) {
					return {
