...
class Seo_url_model extends CI_Model {
	public function addSeoUrl($data) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "seo_url` SET store_id = '" . (int)$data['store_id'] . "', language_id = '" . (int)$data['language_id'] . "', query = '" . $this->db->escape_str($data['query']) . "', keyword = '" . $this->db->escape_str($data['keyword']) . "'");
	}

	public function editSeoUrl($seo_url_id, $data) {
		$this->db->query("UPDATE `" . DB_PREFIX . "seo_url` SET store_id = '" . (int)$data['store_id'] . "', language_id = '" . (int)$data['language_id'] . "', query = '" . $this->db->escape_str($data['query']) . "', keyword = '" . $this->db->escape_str($data['keyword']) . "' WHERE seo_url_id = '" . (int)$seo_url_id . "'");
	}
	...
	public function getSeoUrls($data = array()) {
		...
		if (!empty($data['filter_query'])) {
			$implode[] = "`query` LIKE '" . $this->db->escape_str($data['filter_query']) . "'";
		}
		
		if (!empty($data['filter_keyword'])) {
			$implode[] = "`keyword` LIKE '" . $this->db->escape_str($data['filter_keyword']) . "'";
		}
	...
	public function getSeoUrlsByKeyword($keyword) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_url` WHERE keyword = '" . $this->db->escape_str($keyword) . "'");
