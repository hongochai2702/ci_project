...
class Extension_model extends CI_Model {	
	...
	public function uninstall($type, $code) {
		...
		$this->db->query("DELETE FROM `" . DB_PREFIX . "setting` WHERE `code` = '" . $this->db->escape_str($type . '_' . $code) . "'");
	}	

	public function addExtensionInstall($filename, $extension_download_id = 0) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "extension_install` SET `filename` = '" . $this->db->escape_str($filename) . "', `extension_download_id` = '" . (int)$extension_download_id . "', `date_added` = NOW()");
	...
	public function getTotalExtensionInstalls() {
		return $this->db->count_all('extension_install');
	...
	public function addExtensionPath($extension_install_id, $path) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "extension_path` SET `extension_install_id` = '" . (int)$extension_install_id . "', `path` = '" . $this->db->escape_str($path) . "', `date_added` = NOW()");
	}
