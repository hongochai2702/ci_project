...
class ModelCustomerCustomerApproval extends Model {
	public function getCustomerApprovals($data = array()) {
		...
		if (!empty($data['filter_name'])) {
			$sql .= " AND CONCAT(c.`firstname`, ' ', c.`lastname`) LIKE '%" . $this->db->escape_str($data['filter_name']) . "%'";
		}

		if (!empty($data['filter_email'])) {
			$sql .= " AND c.`email` LIKE '" . $this->db->escape_str($data['filter_email']) . "%'";
		}
		...
		if (!empty($data['filter_type'])) {
			$sql .= " AND ca.`type` = '" . $this->db->escape_str($data['filter_type']) . "'";
		}
		
		if (!empty($data['filter_date_added'])) {
			$sql .= " AND DATE(c.`date_added`) = DATE('" . $this->db->escape_str($data['filter_date_added']) . "')";
		}
	...
	public function getTotalCustomerApprovals($data = array()) {
		...
		if (!empty($data['filter_type'])) {
			$implode[] = "ca.`type` = '" . $this->db->escape_str($data['filter_type']) . "'";
		}
		
		if (!empty($data['filter_date_added'])) {
			$implode[] = "DATE(ca.`date_added`) = DATE('" . $this->db->escape_str($data['filter_date_added']) . "')";
		}
