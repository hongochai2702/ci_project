	...
	public function index() {
		...
		if (($this->input->server('REQUEST_METHOD') == 'POST') && $this->validate()) {
			if (!$this->input->get('module_id')) {
				$this->model_setting_module->addModule('banner', $this->input->post());
			} else {
		...
		if (!$this->input->get('module_id')) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('module/banner', 'user_token=' . $this->session->userdata('user_token'), true)
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->lang->line('heading_title'),
				'href' => $this->url->link('module/banner', 'user_token=' . $this->session->userdata('user_token') . '&module_id=' . $this->input->get('module_id'), true)
			);
		}

		if (!$this->input->get('module_id')) {
			$data['action'] = $this->url->link('module/banner', 'user_token=' . $this->session->userdata('user_token'), true);
		} else {
			$data['action'] = $this->url->link('module/banner', 'user_token=' . $this->session->userdata('user_token') . '&module_id=' . $this->input->get('module_id'), true);
		}
