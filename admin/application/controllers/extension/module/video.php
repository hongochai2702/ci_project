	...
	public function index() {
		...
		if (($this->input->server('REQUEST_METHOD') == 'POST') && $this->validate()) {
			if (!$this->input->get('module_id')) {
				$this->model_setting_module->addModule('video', $this->input->post());
			} else {
		...
		if (!$this->input->get('module_id')) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/module/video', 'user_token=' . $this->session->userdata('user_token'), true)
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->lang->line('heading_title'),
				'href' => $this->url->link('extension/module/video', 'user_token=' . $this->session->userdata('user_token') . '&module_id=' . $this->input->get('module_id'), true)
			);
		}

		if (!$this->input->get('module_id')) {
			$data['action'] = $this->url->link('extension/module/video', 'user_token=' . $this->session->userdata('user_token'), true);
		} else {
			$data['action'] = $this->url->link('extension/module/video', 'user_token=' . $this->session->userdata('user_token') . '&module_id=' . $this->input->get('module_id'), true);
		}
		...
		if ($this->input->post('video_id')) {
			...
			$data['video_id'] = $module_info['video_id'];
		} else {
			$data['video_id'] = '';
		}
		...
		$data['videos'] = $this->model_design_video->getVideos();
		...
		$this->load->view('extension/module/video', $data);
	}
