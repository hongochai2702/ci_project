...
class Store extends MX_Controller {
	...
	public function add() {
		...
		if (($this->input->server('REQUEST_METHOD') == 'POST') && $this->validateForm()) {
			$store_id = $this->model_setting_store->addStore($this->input->post());

			$this->load->model('setting/setting_model','model_setting_setting');

			$this->model_setting_setting->editSetting('config', $this->input->post(), $store_id);
	...
	public function edit() {
		$this->lang->load('setting/store_model','model_setting_setting');
		...
		if (($this->input->server('REQUEST_METHOD') == 'POST') && $this->validateForm()) {
			$this->model_setting_store->editStore($this->input->get('store_id'), $this->input->post());

			$this->load->model('setting/setting_model','model_setting_setting');

			$this->model_setting_setting->editSetting('config', $this->input->post(), $this->input->get('store_id'));

			$success_data = $this->session->userdata('success');
			$success_data = $this->lang->line('text_success');

			$this->response->redirect($this->url->link('setting/store', 'user_token=' . $this->session->userdata('user_token') . '&store_id=' . $this->input->get('store_id'), true));
		}
	...
	public function delete() {
		...
		if ($this->input->post('selected') && $this->validateDelete()) {
			...
			foreach ($this->input->post('selected') as $store_id) {
	...
	protected function getList() {
		...
		$data['add'] = $this->url->link('setting/store/add', 'user_token=' . $this->session->userdata('user_token'), true);
		$data['delete'] = $this->url->link('setting/store/delete', 'user_token=' . $this->session->userdata('user_token'), true);

		$data['stores'] = array();

		$data['stores'][] = array(
			'store_id' => 0,
			'name'     => $this->configs->get('config_name') . $this->lang->line('text_default'),
			'url'      => $this->configs->get('config_secure') ? HTTPS_CATALOG : HTTP_CATALOG,
			'edit'     => $this->url->link('setting/setting', 'user_token=' . $this->session->userdata('user_token'), true)
		);
		...
		foreach ($results as $result) {
			$data['stores'][] = array(
				...
				'name'     => $result['name'],
				'url'      => $result['url'],
				'edit'     => $this->url->link('setting/store/edit', 'user_token=' . $this->session->userdata('user_token') . '&store_id=' . $result['store_id'], true)
			);
		}
	...
	protected function getForm() {
		...
		if (!$this->input->get('store_id')) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('setting/store/add', 'user_token=' . $this->session->userdata('user_token'), true)
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->lang->line('text_settings'),
				'href' => $this->url->link('setting/store/edit', 'user_token=' . $this->session->userdata('user_token') . '&store_id=' . $this->input->get('store_id'), true)
			);
		}
		...
		if (!$this->input->get('store_id')) {
			$data['action'] = $this->url->link('setting/store/add', 'user_token=' . $this->session->userdata('user_token'), true);
		} else {
			$data['action'] = $this->url->link('setting/store/edit', 'user_token=' . $this->session->userdata('user_token') . '&store_id=' . $this->input->get('store_id'), true);
		}

		$data['cancel'] = $this->url->link('setting/store', 'user_token=' . $this->session->userdata('user_token'), true);

		if ($this->input->get('store_id') && ($this->input->server('REQUEST_METHOD') != 'POST')) {
			$this->load->model('setting/setting_model','model_setting_setting');

			$store_info = $this->model_setting_setting->getSetting('config', $this->input->get('store_id'));
		}

		$data['user_token'] = $this->session->userdata('user_token');

		if ($this->input->post('config_url')) {
			$data['config_url'] = $this->input->post('config_url');
		...
		if ($this->input->post('config_image') && is_file(DIR_IMAGE . $this->input->post('config_image'))) {
			$data['thumb'] = $this->model_tool_image->resize($this->input->post('config_image'),$this->input->post('config_image'));
		...
		$this->load->model('localisation/location','model_localisation_location');
		...
		if ($this->input->post('config_logo') && is_file(DIR_IMAGE . $this->input->post('config_logo'))) {
			$data['logo'] = $this->model_tool_image->resize($this->input->post('config_logo'),$this->input->post('config_logo'));
		...
		if ($this->input->post('config_icon') && is_file(DIR_IMAGE . $this->input->post('config_icon'))) {
			$data['icon'] = $this->model_tool_image->resize($this->input->post('config_icon'),$this->input->post('config_icon'));
	...
	protected function validateForm() {
		...
		if (!$this->input->post('config_url')) {
			$this->error['url'] = $this->lang->line('error_url');
		}
		...
		if (!empty($this->input->post['config_customer_group_display']) && !in_array($this->input->post['config_customer_group_id'], $this->input->post['config_customer_group_display'])) {
			$this->error['customer_group_display'] = $this->lang->line('error_customer_group_display');
		}
