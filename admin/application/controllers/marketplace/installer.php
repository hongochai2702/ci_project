...
class Installer extends MX_Controller {
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/installer', 'user_token=' . $this->session->userdata['user_token'], true)
		);
		...
		$data['entry_upload'] = $this->lang->line('entry_upload');
		$data['entry_overwrite'] = $this->lang->line('entry_overwrite');
		$data['entry_progress'] = $this->lang->line('entry_progress');
	
		$data['help_upload'] = $this->lang->line('help_upload');
		...
		if ($directories) {
			$data['error_warning'] = $this->lang->line('error_temporary');
		} else {
			$data['error_warning'] = '';
		}
	...
	public function upload() {
		...
		if (!$json) {
			...
			if (strrchr($this->request->files['file']['name'], '.') == '.xml') {
				...
				if (file_exists($file)) {
					$json['step'][] = array(
						'text' => $this->lang->line('text_xml'),
						'url'  => str_replace('&amp;', '&', $this->url->link('extension/installer/xml', 'user_token=' . $this->session->userdata['user_token'], true)),
						'path' => $path
					);

					// Clear temporary files
					$json['step'][] = array(
						'text' => $this->lang->line('text_remove'),
						'url'  => str_replace('&amp;', '&', $this->url->link('extension/installer/remove', 'user_token=' . $this->session->userdata['user_token'], true)),
						'path' => $path
					);
				} else {
			...
			if (strrchr($this->request->files['file']['name'], '.') == '.zip') {
				...
				if (file_exists($file)) {
					...
					if ($zip) {
						// Zip
						$json['step'][] = array(
							'text' => $this->lang->line('text_unzip'),
							'url'  => str_replace('&amp;', '&', $this->url->link('extension/installer/unzip', 'user_token=' . $this->session->userdata['user_token'], true)),
							'path' => $path
						);

						// FTP
						$json['step'][] = array(
							'text' => $this->lang->line('text_ftp'),
							'url'  => str_replace('&amp;', '&', $this->url->link('extension/installer/ftp', 'user_token=' . $this->session->userdata['user_token'], true)),
							'path' => $path
						);
						...
						while ($entry = zip_read($zip)) {
							...
							if (substr($zip_name, 0, 11) == 'install.sql') {
								$json['step'][] = array(
									'text' => $this->lang->line('text_sql'),
									'url'  => str_replace('&amp;', '&', $this->url->link('extension/installer/sql', 'user_token=' . $this->session->userdata['user_token'], true)),
									'path' => $path
								);
							}
							...
							if (substr($zip_name, 0, 11) == 'install.php') {
								$json['step'][] = array(
									'text' => $this->lang->line('text_php'),
									'url'  => str_replace('&amp;', '&', $this->url->link('extension/installer/php', 'user_token=' . $this->session->userdata['user_token'], true)),
									'path' => $path
								);
							}
							...
							if (is_file($file) && substr($zip_name, 0, 13) == 'upload/') {
						...
						$json['error'] = $this->lang->line('error_unzip');
					}
				} else {
					$json['error'] = $this->lang->line('error_file');
				}
	...
	public function ftp() {
		...
		if (!$this->config->get('config_ftp_status')) {
			$json['error'] = $this->lang->line('error_ftp_status');
		}
						...
						foreach ($files as $file) {
							...
							if (is_dir($file)) {
								...
								if (!in_array(basename($destination), $list_data)) {
									if (!ftp_mkdir($connection, $destination)) {
										$json['error'] = sprintf($this->lang->line('error_ftp_directory'), $destination);
									}
								}
							}
							...
							if (is_file($file)) {
								if (!ftp_put($connection, $destination, $file, FTP_BINARY)) {
									$json['error'] = sprintf($this->lang->line('error_ftp_file'), $file);
								}
							}
						}
					} else {
						$json['error'] = sprintf($this->lang->line('error_ftp_root'), $root);
					}
				} else {
					$json['error'] = sprintf($this->lang->line('error_ftp_login'), $this->config->get('config_ftp_username'));
				}

				ftp_close($connection);
			} else {
				$json['error'] = sprintf($this->lang->line('error_ftp_connection'), $this->config->get('config_ftp_hostname'), $this->config->get('config_ftp_port'));
			}
		}
				...
				try {
					...
					if ($code) {
						...
						if ($modification_info) {
							$json['error'] = sprintf($this->lang->line('error_exists'), $modification_info['name']);
						}
					} else {
	...
	public function clear() {
		...
		if (!$json) {
			...
			$json['success'] = $this->lang->line('text_clear');
		}
