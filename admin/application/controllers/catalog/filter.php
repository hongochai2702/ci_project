<?php
class ControllerCatalogFilter extends MX_Controller {
	private $error = array();

	public function index() {
		$this->load->language('admin/catalog/filter');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('catalog/filter', 'user_token=' . $this->session->userdata('user_token') . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('catalog/filter', 'user_token=' . $this->session->userdata('user_token') . $url, true)
		);

		$data['add'] = $this->url->link('catalog/filter/add', 'user_token=' . $this->session->userdata('user_token') . $url, true);
		$data['delete'] = $this->url->link('catalog/filter/delete', 'user_token=' . $this->session->userdata('user_token') . $url, true);
		...
		foreach ($results as $result) {
			$data['filters'][] = array(
				...
				'edit'            => $this->url->link('catalog/filter/edit', 'user_token=' . $this->session->userdata('user_token') . '&filter_group_id=' . $result['filter_group_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('catalog/filter', 'user_token=' . $this->session->userdata('user_token') . '&sort=fgd.name' . $url, true);
		$data['sort_sort_order'] = $this->url->link('catalog/filter', 'user_token=' . $this->session->userdata('user_token') . '&sort=fg.sort_order' . $url, true);
		...
		$pagination->url = $this->url->link('catalog/filter', 'user_token=' . $this->session->userdata('user_token') . $url . '&page={page}', true);

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->lang->line('text_pagination'), ($filter_total) ? (($page - 1) * $this->configs->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->configs->get('config_limit_admin')) > ($filter_total - $this->configs->get('config_limit_admin'))) ? $filter_total : ((($page - 1) * $this->configs->get('config_limit_admin')) + $this->configs->get('config_limit_admin')), $filter_total, ceil($filter_total / $this->configs->get('config_limit_admin')));
	...
	protected function getForm() {
		$data['text_form'] = !isset($this->request->get['filter_id']) ? $this->lang->line('text_add') : $this->lang->line('text_edit');
		...
		if (!isset($this->request->get['filter_group_id'])) {
			$data['action'] = $this->url->link('catalog/filter/add', 'user_token=' . $this->session->userdata('user_token') . $url, true);
		} else {
			$data['action'] = $this->url->link('catalog/filter/edit', 'user_token=' . $this->session->userdata('user_token') . '&filter_group_id=' . $this->request->get['filter_group_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('catalog/filter', 'user_token=' . $this->session->userdata('user_token') . $url, true);
	...
	protected function validateForm() {
		...
		foreach ($this->request->post['filter_group_description'] as $language_id => $value) {
			if ((utf8_strlen($value['name']) < 1) || (utf8_strlen($value['name']) > 64)) {
				$this->error['group'][$language_id] = $this->lang->line('error_group');
			}
		}
			...
			foreach ($this->request->post['filter'] as $filter_id => $filter) {
				foreach ($filter['filter_description'] as $language_id => $filter_description) {
					if ((utf8_strlen($filter_description['name']) < 1) || (utf8_strlen($filter_description['name']) > 64)) {
						$this->error['filter'][$filter_id][$language_id] = $this->lang->line('error_name');
					}
				}
			}
		}
