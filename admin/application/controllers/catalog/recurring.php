<?php
class ControllerCatalogRecurring extends MX_Controller {
	private $error = array();

	public function index() {
		$this->load->language('admin/catalog/recurring');

		$this->document->setTitle($this->lang->line('heading_title'));

		$this->load->model('admin/catalog/recurring');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('catalog/recurring', 'user_token=' . $this->session->userdata('user_token') . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('catalog/recurring', 'user_token=' . $this->session->userdata('user_token') . $url, true)
		);

		$data['add'] = $this->url->link('catalog/recurring/add', 'user_token=' . $this->session->userdata('user_token') . $url, true);
		$data['copy'] = $this->url->link('catalog/recurring/copy', 'user_token=' . $this->session->userdata('user_token') . $url, true);
		$data['delete'] = $this->url->link('catalog/recurring/delete', 'user_token=' . $this->session->userdata('user_token') . $url, true);
		...
		foreach ($results as $result) {
			$data['recurrings'][] = array(
				...
				'edit'         => $this->url->link('catalog/recurring/edit', 'user_token=' . $this->session->userdata('user_token') . '&recurring_id=' . $result['recurring_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('catalog/recurring', 'user_token=' . $this->session->userdata('user_token') . '&sort=pd.name' . $url, true);
		$data['sort_sort_order'] = $this->url->link('catalog/recurring', 'user_token=' . $this->session->userdata('user_token') . '&sort=p.sort_order' . $url, true);
		...
		$pagination->url = $this->url->link('catalog/recurring', 'user_token=' . $this->session->userdata('user_token') . $url . '&page={page}', true);

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->lang->line('text_pagination'), ($recurring_total) ? (($page - 1) * $this->configs->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->configs->get('config_limit_admin')) > ($recurring_total - $this->configs->get('config_limit_admin'))) ? $recurring_total : ((($page - 1) * $this->configs->get('config_limit_admin')) + $this->configs->get('config_limit_admin')), $recurring_total, ceil($recurring_total / $this->configs->get('config_limit_admin')));
	...
	protected function getForm() {
		$data['text_form'] = !isset($this->request->get['recurring_id']) ? $this->lang->line('text_add') : $this->lang->line('text_edit');
		...
		if (!isset($this->request->get['recurring_id'])) {
			$data['action'] = $this->url->link('catalog/recurring/add', 'user_token=' . $this->session->userdata('user_token') . $url, true);
		} else {
			$data['action'] = $this->url->link('catalog/recurring/edit', 'user_token=' . $this->session->userdata('user_token') . '&recurring_id=' . $this->request->get['recurring_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('catalog/recurring', 'user_token=' . $this->session->userdata('user_token') . $url, true);
		...
		$data['frequencies'][] = array(
			'text'  => $this->lang->line('text_semi_month'),
			'value' => 'semi_month'
		);
	...
	protected function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $recurring_id) {
			$product_total = $this->product_model->getTotalProductsByProfileId($recurring_id);
