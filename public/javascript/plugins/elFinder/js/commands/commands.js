...
elFinder.prototype.commands.mutiimage = function() {
    this.title = 'Insert Image(s)';
    this.alwaysEnabled = true;
    this.updateOnSelect = false;
    this.getstate = function(sel) {
    ...
    this.exec = function(hashes) {
        ...
        $.each(this.files(hashes), function(i, file) {
            var fileName =fm.url(file.hash)
            fileName = fileName.replace(/\\/g,"/");
            if(tinymce.activeEditor.id!=undefined && tinymce.settings.app_default=='cdv'){ //default
                if(file.mime=='image/png' || file.mime=='image/jpeg' || file.mime=='image/gif'){
                        tinymce.activeEditor.execCommand('mceInsertContent',false,"<img src='"+fileName+"'/>");
                }else{
                        tinymce.activeEditor.execCommand('mceInsertContent',false,"<a href='"+fileName+"'/>"+fileName+"</a>");
                }
            }
        });
        if($('#modal-image').length){
