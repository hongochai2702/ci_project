...
* 
* https://github.com/hitech/hitech/wiki/Events-(script-notifications)-2.2.x.x
*/
class Event {
	protected $registry;
 	...
 	*/	
	public function unregister($trigger, $routing) {
		foreach ($this->data as $key => $value) {
			if ($trigger == $value['trigger'] && $value['action']->getId() == $routing) {
				unset($this->data[$key]);
			}
		}			
	}
